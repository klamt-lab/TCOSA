...
def cosa_ratio_test(anaerobic: bool, expanded: bool, activate_reactions: List[str]=[], growth_epsilon: float = 0.01, c_source: str="glucose") -> None:
    """Perform (ATP/ADP) ratio variability analysis under the given settings.

    Args:
        anaerobic (bool): Is is anaerobic (no oxygen)?
        expanded (bool): Is it a 2-cofactor (False) or 3-cofactor (True) model?
        activate_reactions (List[str]): If reaction IDs are given, they are forcibly activated. Defaults to [].
    ...
    for concentrations in concentration_scenarios:
        ...
        for target in ("OPTMDF", "OPTSUBMDF"):
            ...
            for ratio in ratios:
                ylabel = ""
                title = f'Ratios range of ({ratio[0].upper()} '\
                        f'to ({ratio[1].upper()})'
                ...
                figurename = f'2D_{ratio[0].replace("_tcosa_c", "").upper()}_to_{ratio[1].replace("_tcosa_c", "").upper()}_{target}_{concentrations}{activate_suffix}.jpg'
                ...
                ratio_test_data[figurename] = {
                    "min_ratios": min_ratios,
                    "max_ratios": max_ratios,
                    "plotted_growth_rates": plotted_growth_rates,
                }
                json_write(f"./cosa/results{suffix}/ratio_test_data{activate_suffix}.json", ratio_test_data)

    with open(f"./cosa/results{suffix}/figures/2D_report{activate_suffix}.txt", "w", encoding="utf-8") as f:
        f.write(report)

    ratio_test_data = json_load(f"./cosa/results{suffix}/ratio_test_data{activate_suffix}.json")

    for figurename in ratio_test_data.keys():
        plotted_growth_rates = ratio_test_data[figurename]["plotted_growth_rates"]
        min_ratios = ratio_test_data[figurename]["min_ratios"]
        max_ratios = ratio_test_data[figurename]["max_ratios"]
        plt.plot(
...
cosa_ratio_test(anaerobic=False, expanded=False, activate_reactions=["PYK"])
# cosa_ratio_test(anaerobic=True, expanded=False)
# cosa_ratio_test(anaerobic=False, expanded=False, c_source="acetate")
