...
def cosa_multi_flux_sampling(num_samplings: int):
    ...
    ensure_folder_existence(f"./cosa/results{suffix}/mult_flux_samplings3")
    ...
    for round in range(10):
        ...
        for condition_counter in range(len(conditions)):
            ...
            print("Set TCOSA integer cut(s)")
            current_cut = 0
            for old_minimal_tcosa_reaction_set in minimal_tcosa_reaction_sets:
                z_sum_expression = 0.0
                z_sum = len(old_minimal_tcosa_reaction_set)
                for old_tcosa_reaction in old_minimal_tcosa_reaction_set:
                    z_var = multoptmdf_base_variables["z_var_"+old_tcosa_reaction]
                    z_sum_expression += 1.0 * z_var
                integer_cut_var = pulp.LpVariable(
                    name=f"integer_cut_{current_cut}",
                    lowBound=-1e6,
                    upBound=z_sum-1,
            ...
            if optsubmdfpathway_result["status"] != "Optimal":
                ...
                has_error = True
                break
            current_min_z_sum = optsubmdfpathway_result["values"]["global_tcosa_z_var_sum"]
            print("Global min TCOSA Z sum:", current_min_z_sum)
            ...
            ### PHASE 2: z_var difference minmization
            print("PHASE 2")
            multoptmdf_base_variables["global_tcosa_z_var_sum"].bounds(
                0.0,
                current_min_z_sum,
            )
            ...
            difference_expression = 0.0
            for tcosa_reaction_id in get_all_tcosa_reaction_ids(cobra_model):
                is_in_minimal_tcosa_reaction_set = 1.0 if (tcosa_reaction_id in minimal_tcosa_reaction_set) else 0.0
                diff_var = pulp.LpVariable(
                    name=f"diff_{tcosa_reaction_id}",
                    lowBound=0.0,
                    upBound=1.0,
                    cat=pulp.LpContinuous,
                )
                multoptmdf_base_problem += diff_var >= multoptmdf_base_variables["z_var_"+tcosa_reaction_id] - is_in_minimal_tcosa_reaction_set
                multoptmdf_base_problem += diff_var >= -(multoptmdf_base_variables["z_var_"+tcosa_reaction_id] - is_in_minimal_tcosa_reaction_set)
                difference_expression += diff_var
            diff_sum = pulp.LpVariable(
                name="diff_sum",
                lowBound=0.0,
                upBound=float("inf"),
                cat=pulp.LpContinuous,
            )
            multoptmdf_base_problem += diff_sum == difference_expression
            second_phase_result = perform_variable_minimization(
                multoptmdf_base_problem,
                "diff_sum",
            )
            print("DIFF_SUM", second_phase_result["values"]["diff_sum"])
            for tcosa_reaction_id in get_all_tcosa_reaction_ids(cobra_model):
                z_var_id = "z_var_"+tcosa_reaction_id
                if second_phase_result["values"][z_var_id] > 1e-3:
                    minimal_tcosa_reaction_set.append(z_var_id.replace("z_var_", ""))

            minimal_tcosa_reaction_set = list(set(minimal_tcosa_reaction_set))

        json_write(f"minimal_tcosa_reactions_round{round}.json", minimal_tcosa_reaction_set)
        minimal_tcosa_reaction_sets.append(minimal_tcosa_reaction_set)
    ####
    ...
    for num_sampling in range(num_samplings):
        ...
        sampling_json_path = f"./cosa/results{suffix}/mult_flux_samplings3/OPTSUBMDF_{concentration_scenario}_FLEXIBLE_SAMPLING_{num_sampling}.json"
