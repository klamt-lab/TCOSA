...
from optimization import perform_variable_maximization, perform_variable_minimization, solve_current_problem, add_objective, perform_optimization
    ...
    for concentration_scenario in ("STANDARDCONC", "VIVOCONC"):
        ...
        for nadx_scenario in nadx_scenarios:
            ...
            while True: # (used_growth > 0.05) and (used_growth > original_used_growth - step_size * step_number):
                ...
                last_working_OptMDF = 0.01
                current_min_OptMDF = 0.01
                for step_size in [5.0, 1.0, 0.5, 0.1, 0.05]:
                    current_min_OptMDF = last_working_OptMDF
                    while True:
                        current_min_OptMDF += step_size
                        print("Current MIN OptMDF:", current_min_OptMDF)
                        optmdfpathway_base_variables["var_B"].bounds(
                            current_min_OptMDF,
                            1e12
                        )
                        optmdfpathway_result = perform_optimization(
                            optmdfpathway_base_problem,
                            [(0.0, biomass_reaction_id)],
                            "min",
                            optmdfpathway_base_variables
                        )
                        if optmdfpathway_result["status"] == "Optimal":
                            last_working_OptMDF = current_min_OptMDF
                            last_working_solution = copy.deepcopy(optmdfpathway_result)
                        else:
                            break
                optmdfpathway_result = last_working_solution
                ...
                last_working_OptSubMDF = 0.01
                current_min_OptSubMDF = 0.01
                for step_size in [5.0, 1.0, 0.5, 0.1, 0.05]:
                    current_min_OptSubMDF = last_working_OptSubMDF
                    while True:
                        current_min_OptSubMDF += step_size
                        print("Current MIN OptSubMDF:", current_min_OptSubMDF)
                        optmdfpathway_base_variables["var_B2"].bounds(
                            current_min_OptSubMDF,
                            1e12
                        )
                        optsubmdfpathway_result = perform_optimization(
                            optmdfpathway_base_problem,
                            [(0.0, biomass_reaction_id)],
                            "min",
                            optmdfpathway_base_variables
                        ...
                        if optsubmdfpathway_result["status"] == "Optimal":
                            last_working_OptSubMDF = current_min_OptSubMDF
                            last_working_submdf_solution = copy.deepcopy(optsubmdfpathway_result)
                        else:
                            break
                optsubmdfpathway_result = last_working_submdf_solution
